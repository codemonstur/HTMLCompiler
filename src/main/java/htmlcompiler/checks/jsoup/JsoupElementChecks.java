...
public enum JsoupElementChecks {;
    ...
    public interface JsoupElementCheck {
        void checkElement(Logger log, File file, Element element);
    }

    public static void hasStyleAttribute(final Logger log, final File file, final Element element) {
    ...
    public static void hasUppercaseTagsOrAttributes(final Logger log, final File file, final Element element) {
        if (hasUppercase(element.tagName())) {
            log.warn("File " + file.getPath() + " contains a tag " + element.tagName() + " with upper case letters");
        }
    ...
    public static void missingAltForImages(final Logger log, final File file, final Element element) {
        if ("img".equals(element.tagName()) && isNullOrEmpty(element.attr("alt")))
            log.warn("File " + file.getPath() + " contains an img tag without an alt attribute");
    }

    public static void missingPlaceholderForInputs(final Logger log, final File file, final Element element) {
        if ("input".equals(element.tagName()) && isNullOrEmpty(element.attr("placeholder")))
            log.warn("File " + file.getPath() + " contains an input tag without a placeholder attribute");
    }

    public static void dontUseMarquee(final Logger log, final File file, final Element element) {
        if ("marquee".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains a marquee tag");
    }

    public static void dontUseBlink(final Logger log, final File file, final Element element) {
        if ("blink".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains a blink tag");
    }

    public static void dontUseBold(final Logger log, final File file, final Element element) {
        if ("b".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains a b tag, rewrite with strong");
    }

    public static void dontUseItalic(final Logger log, final File file, final Element element) {
        if ("i".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains an i tag, rewrite with em");
    }

    public static void dontUseStrong(final Logger log, final File file, final Element element) {
        if ("strong".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains a strong tag, rewrite with b");
    }

    public static void dontUseEm(final Logger log, final File file, final Element element) {
        if ("em".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains an em tag, rewrite with i");
    }

    public static void dontUseStyling(final Logger log, final File file, final Element element) {
        if ("b".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains a b tag, use CSS");

        if ("i".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains an i tag, use CSS");

        if ("strong".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains a strong tag, use CSS");

        if ("em".equalsIgnoreCase(element.tagName()))
            log.warn("File " + file.getPath() + " contains an em tag, use CSS");
    }

    public static void marginWidthInBody(final Logger log, final File file, final Element element) {
        if ("body".equalsIgnoreCase(element.tagName()) && element.hasAttr("marginwidth"))
            log.warn("File " + file.getPath() + " has a body tag that uses marginwidth attribute");
    }

    public static void alignAttributeContainsAbsmiddle(final Logger log, final File file, final Element element) {
        if ("absmiddle".equalsIgnoreCase(element.attr("align")))
            log.warn("File " + file.getPath() + " has an align attribute with absmiddle as value");
    }

    public static void hasBorderAttribute(final Logger log, final File file, final Element element) {
        if (element.hasAttr("border"))
            log.warn("File " + file.getPath() + " has a tag with a border attribute");
    }
    ...
    public static void hasDeprecatedTag(final Logger log, final File file, final Element element) {
        if (deprecated_tags.contains(element.tagName()))
            log.warn("File " + file.getPath() + " has a deprecated tag " + element.tagName());
    }
    ...
    public static void hasDeprecatedAttribute(final Logger log, final File file, final Element element) {
        for (final var attribute : element.attributes()) {
            final Set<String> tags = deprecated_attributes.get(attribute.getKey());
            if (tags != null && tags.contains(element.tagName())) {
                log.warn("File " + file.getPath() + " has a deprecated attribute " + attribute.getKey() + " for tag " + element.tagName());
            }
        }
    }

    public static void scriptWithHardcodedNonce(final Logger log, final File file, final Element element) {
        if ("script".equalsIgnoreCase(element.tagName()) && !isNullOrEmpty(element.attr("nonce")))
            log.warn("File " + file.getPath() + " has a script with a hardcoded nonce attribute");
    }

    public static void styleWithHardcodedNonce(final Logger log, final File file, final Element element) {
    ...
    public static void labelWithForAttribute(final Logger log, final File file, final Element element) {
        if ("label".equalsIgnoreCase(element.tagName()) && element.hasAttr("for"))
            log.warn("File " + file.getPath() + " has a label tag with a for attribute, use <label>Text<input></label>");
    }

    // Warn on form elements without validation; https://html.spec.whatwg.org/multipage/forms.html#client-side-form-validation
    public static void inputWithoutMaxLength(final Logger log, final File file, final Element element) {
        if ( "input".equalsIgnoreCase(element.tagName())
          && "text".equalsIgnoreCase(element.attr("type"))
          && !element.hasAttr("maxlength")) {
            log.warn("File " + file.getPath() + " has a text input without a maxlength attribute");
        }
    }
    ...
    public static void hasEventHandlerAttribute(final Logger log, final File file, final Element element) {
        for (final var attribute : element.attributes()) {
            if (event_handler_attributes.contains(attribute.getKey().toLowerCase()))
                log.warn("File " + file.getPath() + " has a " + element.tagName() + " tag with an event handler attribute " + attribute.getKey());
        }
    }

    public static void isValidTag(final Logger log, final File file, final Element element) {
        if (!known_tags.contains(element.tagName().toLowerCase()))
            log.warn("File " + file.getPath() + " contains an unknown tag " + element.tagName());
    }
