...
public enum JsoupElementChecks {;
    ...
    public static void missingInputType(final Logger log, final ChecksConfig config, final Path file, final Element element) {
        if (!"input".equals(element.tagName())) return;
        final var type = element.attr("type");
        if (type == null || type.isEmpty()) {
            log.warn("File " + file + " contains an <input> tag without a type attribute");
        }
    }

    public static void unknownInputType(final Logger log, final ChecksConfig config, final Path file, final Element element) {
        if (!"input".equals(element.tagName())) return;
        final var type = element.attr("type");
        ...
        log.warn("File " + file + " contains an <input> tag with unknown type attribute '" + type + "'");
    }
    ...
    public static void scriptWithHardcodedNonce(final Logger log, final ChecksConfig config, final Path file, final Element element) {
        if ("script".equalsIgnoreCase(element.tagName()) && !isNullOrEmpty(element.attr("nonce")))
            log.warn("File " + file + " has a <script> tag with a hardcoded nonce attribute");
    }
    ...
    public static void styleWithHardcodedNonce(final Logger log, final ChecksConfig config, final Path file, final Element element) {
        if ("style".equalsIgnoreCase(element.tagName()) && !isNullOrEmpty(element.attr("nonce")))
            log.warn("File " + file + " has a <style> tag with a hardcoded nonce attribute");
    }
